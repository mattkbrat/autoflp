...
const formName: Form = 'Joint Tenancy';

function tenancyDataCompiler(deal: DealWithRelevant) {
...
async function generateJointTenancy({
  dealId,
  fullDeal,
  output,
}: {
  ...
  try {
    ...
    const appFormObj = tenancyDataCompiler(fullDeal);
...
export default generateJointTenancy;
