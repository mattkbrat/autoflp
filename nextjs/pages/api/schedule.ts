...
import { amortizationSchedule } from '@/utils/finance/amortization';
import {
  FinanceCalcResult,
  FinanceHistory,
...
export type ScheduleType = ScheduleRow[];
...
async function fetchCustomerAmortizationSchedule(
  ...
  if (id !== 'undefined' && id !== undefined) {
    if (typeof id !== 'string') {
      throw new Error('id of the deal must be passed as a string');
    }
    const fetchedPayments = await getDealPayments({ deal: id });
